...
class kmsRequestV5(kmsBase):
    ...
    def decryptRequest(self, request):
        ...
        moo.aes.v6 = self.v6
        decrypted = moo.decrypt(encrypted, 256, moo.ModeOfOperation["CBC"], self.key, moo.aes.KeySize["SIZE_128"],
                                iv)  # *2to3*
    ...
    def encryptResponse(self, request, decrypted, response):
        ...
        for i in range(0, 16):
            randomStuff[i] = (bytearray(decrypted['salt'].encode('latin-1'))[i] ^ iv[i] ^ randomSalt[
                i]) & 0xff  # *2to3*
        ...
        moo = aes.AESModeOfOperation()
        moo.aes.v6 = self.v6
        mode, orig_len, crypted = moo.encrypt(padded, moo.ModeOfOperation["CBC"], self.key, moo.aes.KeySize["SIZE_128"],
                                              iv)  # *2to3*
    ...
    def generateRequest(self, requestBase):
        ...
        moo = aes.AESModeOfOperation()
        moo.aes.v6 = self.v6
        dsalt = moo.decrypt(esalt, 16, moo.ModeOfOperation["CBC"], self.key, moo.aes.KeySize["SIZE_128"],
                            esalt)  # *2to3*

...
class kmsBase:
    ...
    skuIds = {
        ...
        uuid.UUID(
            "8860fcd4-a77b-4a20-9045-a150ff11d609"): "Windows Server 2012 Single Language / Windows 8 Core Single Language",
        uuid.UUID(
            "9d5584a2-2d85-419a-982c-a00888bb9ddf"): "Windows Server 2012 Country Specific / Windows 8 Core Country Specific",
        ...
        uuid.UUID(
            "b148c3f4-6248-4d2f-8c6d-31cce7ae95c3"): "Windows Server 2012 Single Language / Windows 8 Core Single Language [RC]",
        uuid.UUID(
            "c7a8a09a-571c-4ea8-babc-0cbe4d48a89d"): "Windows Server 2012 Country Specific / Windows 8 Core Country Specific [RC]",
...
applicationId TEXT, skuId TEXT, licenseStatus TEXT, lastRequestTime INTEGER, kmsEpid TEXT, requestCount INTEGER)")
        ...
        logging.debug("KMS Request Bytes: \n%s\n" % justify(
            binascii.b2a_hex(str(kmsRequest).encode('latin-1')).decode('utf-8')))  # *2to3*
    ...
    def createKmsResponse(self, kmsRequest):
        ...
        if not self.config["epid"]:
            response["kmsEpid"] = kmsPidGenerator.epidGenerator(kmsRequest['applicationId'].get(),
                                                                kmsRequest['versionMajor'], self.config["lcid"]).encode(
                'utf-16le')
        else:
